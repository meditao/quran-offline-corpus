#!/usr/bin/env python3
"""Tek kök için QAC dağılımı + lemma/yüzey biçimleri + varsa Sami kognat kayıtlarını gösterir."""
...
def main() -> None:
    ...
    parser.add_argument("--root", required=True, help="QAC Buckwalter ROOT, e.g. Slw, Amn, wqy, Hkm")
    ...
    if COGNATES.exists():
        with COGNATES.open("r", encoding="utf-8-sig", newline="") as f:
            cognates = [row for row in csv.DictReader(f, delimiter="\t") if (row.get("quran_root_bw") or "").strip() == args.root]

    print(f"Root: {args.root} / {meta['root_bw_arabic']}")
    print(f"QAC distribution: {meta['word_occurrences']} word occurrences / {meta['ayah_count']} ayat / {meta['surah_count']} surahs")
    ...
    if not cognates:
        print("  (none populated yet; this does not mean no cognates exist)")
    for row in cognates:
        print(f"  - {row['language']} [{row['dialect_period']}] {row['cognate_script']} ({row['cognate_translit']}) — {row['gloss']}")
        print(f"    source={row['source_name']} {row['source_locator']} | phonology={row['phonological_fit']} | semantics={row['semantic_fit']} | confidence={row['confidence']} | effect={row['effect_on_quran_analysis']}")

    if args.limit > 0:
        shown = occurrences[:args.limit]
        print(f"\nFirst occurrences: {len(shown)} / {len(occurrences)}")
        for row in shown:
            print(f"  {row['location']:<12} {row['form_arabic']:<20} lemma={row['lemmas_arabic'] or '-'} pos={row['pos'] or '-'}")
