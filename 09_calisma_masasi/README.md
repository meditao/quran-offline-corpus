...
Durum: **Aşama 1** (`veri.py`, `tara.py`, `kayit.py`, sağlama testleri). Python 3.10+; dış bağımlılık yok.
...
- Kökler çıktıda ayrık, kayıpsız Latin biçimde gösterilir: `Slw → ṣ-l-v`, `fTr → f-ṭ-r`, `rwH → r-v-ḥ`, `Amn → e-m-n`.
  Noktalı harfler ص/س, ط/ت, ح/ه/خ ayrımını korur (`fTr` ile `ftr` ayrı köklerdir).
